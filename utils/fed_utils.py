...
def test_loop(device: str, dataloader, model, loss_fc):
    model.eval()
    with torch.no_grad():
        size = len(dataloader.dataset)
...
def test_loop_with_logits(device: str, dataloader, model, loss_fc):
    with model.eval():
        size = len(dataloader.dataset)
        num_batches = len(dataloader)
        test_loss, correct = 0, 0
...
def to_tensor(data_pairs: dict):
    data_pairs['x'] = torch.tensor(data_pairs['x'], dtype=torch.float32)
...
class GeneralNLPDataset(Dataset):
    def __init__(self, images, labels): # words: 1d words
        self.images = images.detach().int()
        self.labels = labels.detach()
...
def init_model(model, channel, num_classes, im_size=(32, 32)):
    ...
    if model == 'MLP':
        ...
        net = ResNet8(channel=channel, num_classes=num_classes)
    elif model == 'ResNet18':
        ...
        net = MobileNet(width_multiplier=1, class_num=100)
...
def epoch_(mode, dataloader, net, optimizer, criterion, aug, dc_aug_param, device, dsa, dsa_strategy, dsa_param):
    loss_avg, acc_avg, num_exp = 0, 0, 0
    net = net.to(device)
