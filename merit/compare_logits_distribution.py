...
def compute_logits(net, loader):
    ...
    for inputs, targets in tqdm(loader):
        ...
        for l in batch_logits:
            for it in l:
                all_logits.append(it.detach().cpu())
...
if __name__ == '__main__':
    ...
    m1_train_logits = compute_logits(m1, train_loader).flatten()
    m2_train_logits = compute_logits(m2, train_loader).flatten()
    print(f'\t{get_time()}Compute test logits for {len(dst_test)} samples')
    m1_test_logits = compute_logits(m1, test_loader).flatten()
    m2_test_logits = compute_logits(m2, test_loader).flatten()
    ...
    for clz in range(num_classes):
        m1_class_level_logits_on_test[clz] = compute_logits(m1, class_testloaders[clz])
        m2_class_level_logits_on_test[clz] = compute_logits(m2, class_testloaders[clz])
    ...
    # testset_unlearning_class_idx = None
    for clz in unlearning_class:
        if trainset_unlearning_class_idx == None:
            ...
            # testset_unlearning_class_idx = TD_dst_test.labels == clz
        else:
            trainset_unlearning_class_idx = trainset_unlearning_class_idx | unlearning_client_trainset.labels == clz
            # testset_unlearning_class_idx = testset_unlearning_class_idx | TD_dst_test.labels == clz

    trainset_remaining_class_idx = None
    # testset_remaining_class_idx = None
    for clz in remaining_class:
        if trainset_remaining_class_idx == None:
            ...
            # testset_remaining_class_idx = TD_dst_test.labels == clz
        else:
            trainset_remaining_class_idx = trainset_remaining_class_idx | unlearning_client_trainset.labels == clz
            # testset_remaining_class_idx = testset_remaining_class_idx | TD_dst_test.labels == clz
    ...
    ## Training:
    # Unlearning logits:
    m1_unlearn_train_logits = compute_logits(m1, unlearn_train_dataloader)
    m2_unlearn_train_logits = compute_logits(m2, unlearn_train_dataloader)
    # sns.histplot(m1_unlearn_train_logits, color="skyblue", label="Naive Full Set Retrain", kde=True)
    sns.histplot(m1_unlearn_train_logits, color="skyblue", label=args.m1_label, kde=True)
    sns.histplot(m2_unlearn_train_logits, color="red", label=args.m2_label, kde=True)
    plt.legend()
    plt.xlim(-10, 10)
    plt.savefig('Logtis Diff on Unlearning Training Set.png')
    plt.cla()

    # Retaining logits:
    m1_remain_train_logits = compute_logits(m1, remain_train_dataloader)
    m2_remain_train_logits = compute_logits(m2, remain_train_dataloader)
    # sns.histplot(m1_remain_train_logits, color="skyblue", label="Naive Full Set Retrain", kde=True)
    sns.histplot(m1_remain_train_logits, color="skyblue", label=args.m1_label, kde=True)
    sns.histplot(m2_remain_train_logits, color="red", label=args.m2_label, kde=True)
    plt.legend()
    plt.xlim(-10, 10)
    plt.savefig('Logtis Diff on Retaining Training Set.png')
    plt.cla()
